...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS diff FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING diff >= 0;"
end
